...
async def create_completion(request: CompletionRequest):
    try:
        ...
        logger.info(f"请求的model是 {request.model}")
        logger.info(f"当前模型为: {rkllm_model}")
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    try:
        ...
        if rkllm_model is None:
            initialize_model(request.model)
            last_model_name = request.model

        # 检查是否需要重新加载模型
        if request.model != last_model_name:
            # 调用垃圾回收
            gc.collect()
            logger.info(f"模型需要重新加载，上次模型: {last_model_name}, 本次模型: {request.model}")
            initialize_model(request.model)
            last_model_name = request.model  # 更新上次请求的模型名称
        else:
            logger.info("模型已加载，跳过重新加载")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--host",default="0.0.0.0", type=str, help="The host to bind the server to.",        
    )
    parser.add_argument(
        "--port",default=8000, type=int, help="The port to bind the server to.",        
    )
