...
import string
import logging
from model_class import available_models, RKLLMLoaderClass  # Import available_models and RKLLMLoaderClass
from model_configs import model_configs  # Import model_configs from the separate file
...
async def create_completion(request: CompletionRequest):
    try:
        ...
        max_tokens = min(request.max_tokens, base_config["max_new_tokens"]) if request.max_tokens is not None else base_config["max_new_tokens"]
        ...
        if prompt_tokens > base_config["max_context_len"]:
            raise HTTPException(status_code=400, detail=f"Token count ({prompt_tokens}) exceeds the model's max_context_len ({base_config['max_context_len']}).")

        # Use the request parameters if provided, otherwise fall back to the model's default configuration
        temperature = request.temperature if request.temperature is not None else base_config["temperature"]
        top_p = request.top_p if request.top_p is not None else base_config["top_p"]
        frequency_penalty = request.frequency_penalty if request.frequency_penalty is not None else base_config["frequency_penalty"]
        ...
        if finish_reason == "length":
            logger.warning(f"Response truncated due to token limit. Prompt tokens: {prompt_tokens}, Completion tokens: {completion_tokens}, Max tokens: {max_tokens}")
